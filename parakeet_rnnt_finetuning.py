...
def check_and_install_dependencies():
    ...
    if missing:
        ...
        print("  pip install git+https://github.com/NVIDIA/NeMo.git#egg=nemo_toolkit[asr]")
        print("="*60)
        return False
...
class TrainingConfig:
    ...
    # Scheduler
    scheduler: str = "CosineAnnealing"  # WarmupAnnealing, CosineAnnealing, NoamAnnealing
        ...
        try:
            ...
            if isinstance(audio, dict):
                ...
                if audio_output_dir:
                    ...
                    logging.warning(f"Sample {idx}: No audio_output_dir provided for in-memory audio")
                    continue
    ...
    with open(manifest_path, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f, 1):
            try:
                ...
                # Text length range
                text_len = len(text)
                stats["text_length_range"]["min"] = min(stats["text_length_range"]["min"], text_len)
                stats["text_length_range"]["max"] = max(stats["text_length_range"]["max"], text_len)
...
def print_manifest_stats(manifest_path: str):
    ...
    try:
        ...
        print(f"  Duration range: {stats['duration_range']['min']:.2f}s - {stats['duration_range']['max']:.2f}s")
        print(f"  Text length range: {stats['text_length_range']['min']} - {stats['text_length_range']['max']} chars")
...
class ParakeetRNNTFineTuner:
    ...
    def load_pretrained_model(self):
        ...
        self.logger.info(f"Loading pre-trained model: {self.config.training.pretrained_model}")
    ...
    def train(self):
        ...
        self.trainer.fit(
            self.model,
            ckpt_path=self.config.training.resume_from_checkpoint
        )
...
def transcribe_with_model(model, audio_paths: Union[str, List[str]], batch_size: int = 4):
    """
    ...
    return model.transcribe(paths2audio_files=audio_paths, batch_size=batch_size)
...
def parse_args():
    ...
    parser.add_argument('--transcriptions', type=str, help='JSON file with transcriptions')
...
def main():
    ...
    if args.mode == 'train':
        ...
        if not manifest_path:
            print("Error: Provide a manifest path (--train-manifest, --val-manifest, or --test-manifest)")
            sys.exit(1)
