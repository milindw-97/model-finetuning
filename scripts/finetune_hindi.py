...
def finetune_parakeet(
    ...
    exp_dir = config.get('exp_manager', {}).get('exp_dir', 'outputs/parakeet-hindi-finetuned')
    ...
    model_name = config.get('model', {}).get('pretrained_name', 'parakeet-rnnt-1.1b-multilingual')
    ...
    with open_dict(cfg):
        ...
        # Validation data
        cfg.validation_ds.manifest_filepath = config['validation_ds']['manifest_filepath']
        ...
        cfg.validation_ds.sample_rate = config['validation_ds'].get('sample_rate', 16000)
    ...
    exp_cfg = {
        ...
        'checkpoint_callback_params': exp_manager_config.get('checkpoint_callback_params', {
            'monitor': 'val_wer',
            'mode': 'min',
            'save_top_k': 3,
            'save_last': True,
        }),
        'create_tensorboard_logger': exp_manager_config.get('create_tensorboard_logger', True),
    ...
    print(f"\nNext steps:")
    print(f"  1. Evaluate: python scripts/evaluate.py --model {exp_dir}/final_model.nemo")
    print(f"  2. Transcribe: python parakeet_rnnt_finetuning.py --mode transcribe --model-path {exp_dir}/final_model.nemo --audio test.wav")
    print("=" * 60)
...
def main():
    ...
    parser.add_argument(
        "--test-manifest",
        type=str,
        help="Override test manifest path"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--no-freeze-encoder",
        action="store_true",
        help="Don't freeze encoder layers"
    )
    parser.add_argument(
        "--resume",
        type=str,
